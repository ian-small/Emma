...
function main()
    ...
    if all_dirs
        ...
        for fasta in fafiles
            ...
            outfile_gff = ~isnothing(args[:GFF_out]) ? joinpath(args[:GFF_out], accession * ".gff") : nothing
            outfile_gb = ~isnothing(args[:GB_out]) ? joinpath(args[:GB_out], accession * ".tbl") : nothing
            outfile_fa = ~isnothing(args[:FA_out]) ? joinpath(args[:FA_out], accession * ".fa") : nothing
            outfile_svg = ~isnothing(args[:SVG_out]) ? joinpath(args[:SVG_out], accession * ".svg") : nothing
            try
                emma(fasta; translation_table=args[:translation_table], rotate_to=args[:rotate_to],
                    outfile_gff=outfile_gff, outfile_gb=outfile_gb, outfile_fa=outfile_fa, outfile_svg=outfile_svg)
            catch e
                @error "$(accession): $e"
            end
        end
    elseif all_files
        emma(args[:FASTA_file]; translation_table=args[:translation_table],
            rotate_to=args[:rotate_to], outfile_gff=args[:GFF_out], outfile_gb=args[:GB_out],
            outfile_fa=args[:FA_out], outfile_svg=args[:SVG_out])
    else
